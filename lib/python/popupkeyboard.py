...
class PopupKeyboard:
    def __init__(
        ...
        if use_coord_buttons:
            self.support_coord_buttons()

        # making it insensitive clears the initial selection region
        self.num_entry.set_state(gtk.STATE_INSENSITIVE)
        self.num_entry.modify_text(gtk.STATE_INSENSITIVE, gtk.gdk.color_parse("black"))
    ...
    def run(self, initial_value="", title=None):
        ...
        if self.location:
            self.dialog.parse_geometry("+%d+%d" % (self.location[0], self.location[1]))
...
if __name__ == "__main__":
    ...
    ct = 100
    while True:
        m.run(initial_value="", title=str(ct))
        result = m.get_result()
