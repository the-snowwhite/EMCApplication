...
def select_widgets(widgets, hal_only=False, output_only=False):
    ...
    for w in widgets:
        ...
        if hal_only and not isinstance(w, _HalWidgetBase):
            continue
        if output_only and not isinstance(
            w,
            (
                gtk.Range,
                gtk.SpinButton,
...
def accessors(w):
    ...
    if isinstance(
        w, (gtk.CheckButton, gtk.ToggleButton, gtk.RadioButton, gtk.ComboBox)
    ):
...
class IniFile(object):
    ...
    def restore_state(self, obj):
        ...
        if self.defaults[IniFile.ini][IniFile.signature] != (
            ...
            warn("signature mismatch in %s -  resetting to default" % (self.filename))
            dbg(
                1,
                "expected: %s, got %s"
                % (
    ...
    def save_state(self, obj):
        ...
        self.config.final_comment = [
            "last update  by %s.save_state() on %s " % (__name__, time.asctime())
        ]
        self.write()
    ...
    def create_default_ini(self):
        ...
        self.config = ConfigObj(
            self.filename, interpolation=False, configspec=self.spec
        )
        vdt = Validator()
        self.config.validate(vdt, copy=True)
        self.config.update(self.defaults)
        self.config.filename = self.filename
        self.config.initial_comment.append(
            "generated by %s.create_default_ini() on %s" % (__name__, time.asctime())
        )
        self.config.write()
    ...
    def read_ini(self):
        ...
        while True:
            try:
                ...
                if results != True:
                    for (section_list, key, error) in flatten_errors(
                        self.config, results
                    ):
                        if key is not None:
                            warn(
                                "key '%s' in section '%s' : %s"
                                % (
                                    key,
                                    ", ".join(section_list),
                            ...
                            warn("section '%s' missing" % ", ".join(section_list))
                    badfilename = self.filename + ".BAD"
                    os.rename(self.filename, badfilename)
                    retries -= 1
                    if retries:
                        raise UselessIniError(
                            "cant make sense of '%s', renamed to '%s'"
    ...
    def __init__(self, filename, defaults, builder):
        defaults[IniFile.ini] = {
            IniFile.signature: "astring",
        ...
        dbg(2, "auto-generated spec:\n%s\nsignature = %s" % (spec, self.signature))
